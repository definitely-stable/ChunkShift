...
`streaming.bytesCopied` is the number of bytes `ChunkingKernel` moved inside its buffer for the source and target together, counted by `ChunkingKernelCounters` in a separate untimed pass so the counter never touches the timed samples; `streaming.bytesCopiedPerInputByte` divides it by the measured bytes. Since the single-buffer topology (A1-F05), reads land directly after the pending chunk and only the pending prefix is moved to the front when the buffer end is reached, so the value is a fraction of the input (the earlier two-buffer topology copied every byte once, 1.0). This is the "bytes copied per GiB" regression signal in `docs/PERFORMANCE.md`. The reference lane does not copy.
...
By default all experiments run in one process, so working-set values are same-process observations and `processPeakRssBytes` is a process-lifetime upper bound over every experiment run so far, not a per-experiment peak. `lab ... --isolate` runs each experiment in its own child process (`lab ... --only <id>`, which can also be run directly) and merges the results in manifest order; the parent refuses a child result with a different schema, experiment, mode or commit. `measurement.processIsolation` records the mode: `none`, `single-experiment` or `per-experiment`.

An isolated peak covers one experiment's process: runtime start-up, corpus and mutation generation, stabilization of that experiment's shape and both lanes. It is still not a per-lane figure, and runtime and generated-corpus memory dominate it: a difference of one 64 KiB chunk buffer (the A1-F05 topology change) is far below its resolution, so buffer-size claims are argued from the exact pooled-buffer sizes, not from RSS. Profile/release memory decisions still require the isolated streaming/file lane owned by M1 measurement work (`PLAN.md` §14).
